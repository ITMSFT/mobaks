...
Інтеграція здійснюється шляхом генерації та вивантаження **одного YML-файлу** на SFTP-сервер. Цей файл містить повну інформацію про каталог, ціни та залишки.

Файл повинен мати **статичне (незмінне) ім'я**.

### 2. 📦 Єдине вивантаження каталогу, цін та залишків (YML)
...
Интеграция осуществляется путем генерации и выгрузки **одного YML-файла** на SFTP-сервер. Этот файл содержит полную информацию о каталоге, ценах и остатках.

Файл должен иметь **статичное** (неизменяемое) **имя**.

### 2. 📦 Единая выгрузка каталога, цен и остатков (YML)
